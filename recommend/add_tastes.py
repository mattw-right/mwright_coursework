    ...
    if most_recent_return.get():
        ...
        if most_recent_return.get()[id][-1].split(':')[1] == 'track': #check if it's a track
            ...
            db.execute(
                ...
                (most_recent_return.get()[id][0], most_recent_return.get()[id][-1], track.return_danceability(), track.return_energy(), track.return_key(), track.return_loudness(), track.return_mode(), track.return_speechiness(), track.return_acousticness(), track.return_instrumentalness(), track.return_liveness(), track.return_valence(), track.return_tempo(), track.return_duration_ms(), track.return_time_signature(), metadata['album']['images'][0]['url'], metadata['album']['artists'][0]['external_urls']['spotify'], metadata['album']['artists'][0]['name'], metadata['preview_url']),
            )
            ...
            db.execute(
                ...
                (most_recent_return.get()[id][0], most_recent_return.get()[id][-1], artist[0], artist[1], artist[2], artist[3], artist[4], artist[5], artist[6], artist[7], artist[8], artist[9], artist[10], artist[11], artist[12], metadata['images'][0]['url'], ', '.join(metadata['genres'])),
            )
        db.commit()
